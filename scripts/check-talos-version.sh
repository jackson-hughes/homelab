#!/usr/bin/env bash
# Verify that the declared Talos version pin and schematic extensions agree
# and that the Image Factory can build that combination. Used by CI
# (.github/workflows/metal.yaml). Does not talk to the cluster.
set -euo pipefail

cd "$(git rev-parse --show-toplevel)"

topf_file="infra/metal/topf.yaml"
schematic_file="infra/metal/schematic.yaml"

if [ ! -f "${topf_file}" ]; then
  echo "error: ${topf_file} is missing" >&2
  exit 1
fi
pin="$(awk '/^talosVersion:/{gsub(/["'\'']/, "", $2); print $2; exit}' "${topf_file}")"
if [[ ! "${pin}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  echo "error: ${topf_file} talosVersion must be a stable tag like v1.13.9, got: ${pin}" >&2
  exit 1
fi
